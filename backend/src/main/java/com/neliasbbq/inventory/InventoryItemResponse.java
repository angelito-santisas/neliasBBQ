...
public record InventoryItemResponse(
    ...
    BigDecimal reorderLevel, BigDecimal unitCost, String status, OffsetDateTime updatedAt, String imageUrl
) {
    static InventoryItemResponse from(InventoryItem item) {
        ...
        return new InventoryItemResponse(item.getId(), item.getName(), item.getSku(), item.getCategory(),
            ...
            item.getPhotoId() == null ? null : "/api/v1/staff/photos/" + item.getPhotoId());
    }
}
