        ...
        <div class="cart-items-list">
          ...
          @for (line of cart.lines(); track line.item.id) {
            <div class="cart-item">
              ...
              <div class="cart-item-info"><h4>{{ line.item.name }}</h4><p>{{ line.item.price | currency:'PHP' }} each</p>
...
export class CartComponent {
  ...
  checkout(): void {
    if (!this.cart.lines().length || this.submitting() || this.cart.exceedsStock() || !this.cart.canOrder()) return;
    this.submitting.set(true);
    this.cart.checkAvailability().pipe(switchMap(checked => {
      if (!checked || !this.cart.canOrder() || this.cart.exceedsStock() || !this.cart.lines().length) {
        this.notifications.show('Availability changed. Review your cart before ordering.');
        return EMPTY;
      }
      return this.api.createOrder(this.cart.toOrderRequest(this.instructions.trim()));
